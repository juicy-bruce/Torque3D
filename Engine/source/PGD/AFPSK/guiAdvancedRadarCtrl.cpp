...
void GuiAdvancedRadarCtrl::drawRotatableBitmap(GFXTextureObject* texture, const RectF &dstRect, const RectF &srcRect, F32 angle, const GFXBitmapFlip in_flip = GFXBitmapFlip_None) {
...
void GuiAdvancedRadarCtrl::drawRotatableBitmap(GFXTextureObject* texture, const Point2I &in_rAt, const GFXBitmapFlip in_flip, const GFXTextureFilterType filter , bool in_wrap /*= true*/, F32 angle /*= 0.0f*/) {
   ...
   drawRotatableBitmap( texture, stretch, subRegion, angle );
}
...
void GuiAdvancedRadarCtrl::drawRotatableBitmapCircle(GFXTextureObject* texture, const Point2F &center, F32 radius, F32 angle) {
   ...
   GFX->drawPrimitive(GFXTriangleStrip, 0, numPoints);
   GFX->drawPrimitive(GFXTriangleStrip, numPoints + 1, numPoints);
				...
				if (shape) {
					...
					if (shape->getShapeIcon() && dStrcmp(shape->getClassName(), "WayPoint") == 0) {
					   arrowPath = shape->getShapeIcon();
						...
						if(texture && (shape->getTeam() == dynamic_cast<ShapeBase *>(control)->getTeam()
							&& shape->getTeam() == 0)) {
