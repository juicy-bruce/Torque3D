...
class GuiAdvancedRadarCtrl : public GuiControl {
   ...
   public:
      ...
      void setImageLayer(S32 layer, const char *name);
		...
		void drawRotatableBitmap(GFXTextureObject* texture, const RectF &dstRect, const RectF &srcRect, F32 angle, const GFXBitmapFlip in_flip);
		void drawRotatableBitmap(GFXTextureObject* texture, const Point2I &in_rAt, const GFXBitmapFlip in_flip, const GFXTextureFilterType filter, bool in_wrap, F32 angle);
		void draw2DCircle(const GFXStateBlockDesc &desc, const Point2F &loc, F32 radius, const ColorI &color);
		void drawRotatableBitmapCircle(GFXTextureObject* texture, const Point2F &center, F32 radius, F32 angle);
